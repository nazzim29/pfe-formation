...
exports.create = (req, res) => {
	let postulation = new Postuler(
		req.session.currentUser._id + "_" + req.body.id_seminaire
	);
	...
	if (req.query.json) {
		Postuler.getAll().then((d) => {
			...
			d.forEach((element) => {
				...
				let f = new Seminaire(element.id.split("_")[1]);
				c.push(
					f.read().then(async () => {
						element.seminaire = {
							id: f._id,
							...
							place: f.place,
							participant: await Postuler.validPostulationBySeminaire(f._id),
						};
					})
				);
			});
		...
		if (!pid && req.session.currentUser._role != "utilisateur")
			return res.render("pages/admin/seminairepostulation");
		if (!pid && req.session.currentUser._role == "utilisateur")
			return res.render("pages/seminairepostulation");
		if (pid) {
			return res.redirect("\\seminaire/" + pid.split("_")[1]);
		}
	}
};
