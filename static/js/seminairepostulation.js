...
table = $("#example")
	.DataTable({
		...
		columnDefs: [
			...
			{
				targets: 1,
				data: "seminaire.titre",
				render: function (data, type, row, meta) {
					return `
                        <div class="flex flex-col">
                            <a href="${row.seminaire.id}" target="_blank"> <h1>${row.seminaire.titre}</h1></a>
                            <div class="flex flex-row flex-nowrap overflow-x-auto w-full">
                            ${row.seminaire.activite
															.map((e) => {
			...
			{
				targets: 2,
				data: "seminaire.participant",
				render: function (data, type, row, meta) {
					return `<span class="text-semibold">${row.seminaire.participant}/${row.seminaire.place}</span>`;
				},
			},
			{
				targets: 3,
				data: "seminaire.date_debut",
				render: function (data, type, row, meta) {
					if (moment(row.seminaire.date_debut).diff(moment()) < 0)
						return `${moment(row.seminaire.date_debut).fromNow()}`;
					return `${moment(row.seminaire.date_debut).toNow()}`;
				},
			},
			{
				targets: 4,
				data: "seminaire.date_fin",
				render: function (data, type, row, meta) {
					return `${moment
						.duration(
							moment(row.seminaire.date_debut).diff(
								moment(row.seminaire.date_fin)
							)
						)
						.humanize()}`;
				},
...
const valider = (id) => {
	...
	xhr.open("post", "\\seminaire/postulation/" + id);
	...
	xhr.send(JSON.stringify({ valider_df: "accepté" }));
};
const refuser = (id) => {
	let xhr = new XMLHttpRequest();
	xhr.open("post", "\\seminaire/postulation/" + id);
	...
	xhr.send(JSON.stringify({ valider_df: "refuser" }));
};
...
$(document).on("click", (e) => {
		...
		switch (data.toggle) {
			case "refuser":
				...
				$(".modal-content .modal-body").html(
					`<p>etes vous sur de vouloir refuser la postulation de <span class="font-semibold user-name">${row.user.nom.toUpperCase()} ${row.user.prenom.toLowerCase()}</span> au seminaire ${
						row.seminaire.titre
					}</p>`
				);
    ...
    <button class="px-4  p-1.5 rounded-lg text-white bg-red-500 hover:bg-red-700 action">Supprimer</button>`);
			...
			case "valider":
				...
				$(".modal-content .modal-body").html(
					`<p>etes vous sur de vouloir valider la postulation de <span class="font-semibold user-name">${row.user.nom.toUpperCase()} ${row.user.prenom.toLowerCase()}</span> au seminaire ${
						row.seminaire.titre
					}</p>`
				);
				$(".modal-content .modal-footer").html(`<button
