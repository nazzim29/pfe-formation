    ...
    columnDefs: [
      {
        ...
        render: function (data, type, row, meta) {
          return `
                    <img class="avatar-sm" src="${row.logo}">
                    <div class="flex flex-col">
                        <a href="\\partenaire/${
                          row.id
                        }" target="_blank">${row.nom.toUpperCase()}</a>    
                    </div>
                  `;
        },
      },
      {
        ...
        data: "type",
        render: function (data, type, row, meta) {
          return `<span class="${row.type}">${row.type}</span>`;
        },
      },
      {
        targets: 2,
...
const modifier = (f) => {
  ...
  form.append("description", $(".modal-content .modal-body #description").val()),
  form.append("logo", $(".modal-content .modal-body #logo")[0].files[0]||null)
  xhr.send(form);
...
const creer = (f) => {
  if (
    !(
      ...
      $(".modal-content .modal-body #type").val() &&
      $(".modal-content .modal-body #description").val() &&
      $(".modal-content .modal-body #logo").val()
    )
  )
    return alert("all field required!!");
  let form = new FormData()
  let xhr = new XMLHttpRequest();
  xhr.open("post", "\\partenaire");
  xhr.onload = () => {
