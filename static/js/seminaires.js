...
const card = (f) => {
	return `<div class="w-full sm:w-1/2 md:w-1/2 xl:w-1/4 p-4">
        <div class="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">
            <div class="p-4">
                <a href="seminaire/${f.id}"><h2 class="mt-2 mb-2  font-bold">${
		f.titre
	}</h2></a>
                ${f.activite
									.map((e) => {
            ...
            <div class="p-4 border-t border-b text-xs text-gray-700">
                ...
                <span class="flex items-center">
                    <i class="material-icons text-gray-900 mr-2">school</i> ${
											f.animateurs.map(e=>e._nom.toUpperCase()+" "+e._prenom.charAt(0).toUpperCase()+e._prenom.slice(1).toLowerCase()).join(', ')
										}</span>
            </div>
            <div class="flex items-center justify-center">
                <button  ${
									f.postuled
										? "disabled"
										: `data-target='postuler' data-toggle='${f.id}'`
								} class="btn-postuler w-full h-full text-bold hover:bg-atblue-light focus:bg-atblue outline-none focus:outline-none px-2 py-1">${
		f.postuled ? "Postulé" : "Postuler"
	}</button>
            </div>
        </div>
    </div>`;
...
const getSeminaire = () => {
	...
	xhr.open("get", "\\seminaire?json=true");
	xhr.responseType = "json";
	xhr.onload = () => {
		console.log(xhr.response);
		let seminaires = xhr.response;
		if (q) {
...
const afficher = (e) => {
    ...
    <div class="bg-white rounded-lg p-1 shadow-xl flex flex-col">
      <p class="text-sm">Pas de seminaire pour l'instant veuillez visiter cette page ultérieurement</p>
...
const postuler = (id) => {
	if (!id) return;
	let xhr = new XMLHttpRequest();
	xhr.open("post", "\\seminaire/postulation");
