...
const modifier = (f) => {
	if (
		!(
			...
			$(".modal-content .modal-body #etablissement").val()
		)
	)
	...
	let form = {
		...
		etablissement: $(".modal-content .modal-body #etablissement").val(),
	};
	let xhr = new XMLHttpRequest();
	xhr.open("POST", "\\formateur/" + f);
    ...
    <button class="px-4  p-1.5 rounded-lg text-white bg-red-500 hover:bg-red-700 action">Supprimer</button>`);
			...
			case "modifier":
				...
				$(".modal-content .modal-body").html(`
                    ...
                    <input
                    class='border px-1 focus:border-atblue outline-none rounded-lg border-gray-100 placeholder-atgreen bg-gray-100'
                    type="text" name="etablissement" id="etablissement" placeholder="etablissement" value='${
											row.etablissement
										}'>
                    </div>
                `);
