...
module.exports = class Formateur extends Model {
	static colref = this.db.collection("seminairepostulertest");
	constructor(id) {
		super();
		this.colref = this.db.collection("seminairepostulertest");
		if (id) {
			this._id = id;
	...
	static async validPostulationBySeminaire(id) {
		return await this.getAll().then((f) => {
			let i = 0;
			f.forEach((element) => {
				if (
					element.id.split("_")[1] == id &&
					element.valider_df == "accepté" &&
					element.valider_superieur == "accepté"
				)
					i++;
			});
			return i;
