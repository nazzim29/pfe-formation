...
module.exports = class Formulaire extends Model {
  static colref = this.db.collection("Seminaireformulaire");
  constructor(id) {
    super();
    this.colref = this.db.collection("Seminaireformulaire");
    if (id) {
      this._id = id;
  ...
  static getAll() {
    return this.colref.get().then((snapshot) => {
      let Seminaireformulaire = [];
      snapshot.forEach((doc) => {
        let t = doc.data();
        t.id = doc.id;
        Seminaireformulaire.push(t);
      });
      return Seminaireformulaire;
    });
  }
